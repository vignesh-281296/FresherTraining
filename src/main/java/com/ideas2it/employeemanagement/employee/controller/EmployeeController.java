...
public class EmployeeController {
	...
	private ModelAndView assignProject(@RequestParam int id,
			@RequestParam(value = "projects", required = false) String[] projectId) {
		ModelAndView modelAndView = new ModelAndView();
		try {
	...
	private ModelAndView updateEmployee(@ModelAttribute("employee") Employee employee,
			@RequestParam("temporary") String address) {
		ModelAndView modelAndView = new ModelAndView();
		try {
			List<Address> addresses = new ArrayList<Address>();
			if (0 == Integer.parseInt(address)) {
	...
	/**
	 * method to get employee page name
	 * 
	 * @return employee page name
	 */
	@GetMapping("/employee")
